...
class ThreadTestCase(TestCase):
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Hola tu cómo vas?")
    ...
    # Se crea un hilo de conversación y el usuario 3 no es parte del hilo
    def test_add_message_from_user_not_in_thread(self):
        self.thread.users.add(self.user1, self.user2)

        message1 = Message.objects.create(user=self.user1, content="Hola tu cómo vas?")
        message2 = Message.objects.create(user=self.user2, content="Bien y tu?")
        message3 = Message.objects.create(user=self.user1, content="Que bueno. Bien gracias")
        message4 = Message.objects.create(user=self.user3, content="Soy un espía")
